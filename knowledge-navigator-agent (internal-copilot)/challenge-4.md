...
After the topic is generated, you need to connect it to your Email Document to Employee flow:

- Open the **EmailDocument** topic in the editor.

- Review the AI-generated conversation flow and identify where all variables are collected (email address, document name, description).

- At the end of the conversation flow, before any final message, click **+** to add a node.

- Select **Call an action** → **Email Document to Employee** flow.

- Map the flow inputs to your topic variables:
   - **EmployeeEmail:** Select the email variable from your topic (look for variable names like `EmailAddress`, `UserEmail`, or similar)
   - **DocumentName:** Select the document name variable (look for `DocumentName`, `Document`, or similar)
   - **DocumentDescription:** Select the description variable (look for `Description`, `DocumentDescription`, or similar)

   > **Tip:** Variable names are auto-generated by AI. Click on each input field and select from the dropdown list of available topic variables.

- After the flow action node, add a **Message** node by clicking **+**:
...
After the topic is generated, you need to connect it to your Send Request to Teams flow:

- Open the **SubmitRequest** topic in the editor.

- Review the AI-generated conversation flow and identify where all variables are collected.

- **Important:** If the AI created a multiple choice question for request type, you need to convert it to text:
   - After the multiple choice question node, click **+** to add a **Set a variable value** node
   - Create a new variable called `RequestTypeText`
   - Set its value to the multiple choice variable (this converts it to string format)

- At the end of the conversation flow, before any final message, click **+** to add a node.

- Select **Call an action** → **Send Request to Teams** flow.

- Map the flow inputs to your topic variables:
   - **EmployeeName:** Select the name variable (look for `EmployeeName`, `Name`, `UserName`, or similar)
   - **EmployeeEmail:** Select the email variable (look for `EmailAddress`, `Email`, or similar)
   - **RequestType:** Select the `RequestTypeText` variable you created (NOT the multiple choice variable)
   - **RequestDetails:** Select the details variable (look for `RequestDetails`, `Details`, `Description`, or similar)

   > **Tip:** If you see a type mismatch error for RequestType, make sure you're using the text variable (`RequestTypeText`), not the multiple choice variable.
