...
Welcome to the Unified Data Foundation with Fabric Hack in a Day! Today, you'll explore how to build an enterprise-grade data lakehouse using Microsoft Fabric's unified analytics platform. Through hands-on challenges, you will implement the Medallion architecture (Bronze-Silver-Gold layers), perform data quality transformations, create dimensional models, and build machine learning models using Fabric's Data Science capabilities.
...
Your mission is to build a **Data Lakehouse Solution** using Microsoft Fabric OneLake that enables the airline to analyze flight loyalty program data and transaction patterns. Using Microsoft Fabric and Power BI, you will design an end-to-end solution that can:
...
- Perform customer segmentation using K-Means clustering within Fabric's Data Science workload
...
- Use Fabric's Data Science workload to build ML models for customer segmentation
- Perform machine learning customer segmentation using K-Means clustering with MLflow
- Write enriched ML data back to Fabric Lakehouse tables
...
- **Challenge 5: Build ML Models with Fabric Data Science**  
  Use Fabric's Data Science workload to create ML experiments, perform customer segmentation with K-Means clustering, track experiments with MLflow, and write enriched data back to the Lakehouse.
